...
class BlockStorageShardingSpec:
    ...
    Attributes:
        ...
        # TODO(ailzhang): these works, are they the best way to represent uneven sharding?
        ...
        # TODO(ailzhang): what is this for?
...
class MemoryShardedDTensor(DTensor):
    ...
    def __torch_dispatch__(cls, func, types, args=(), kwargs=None):  # type: ignore[override]
        ...
        raise NotImplementedError(
            ...
            f"then use scatter_storage() to re-shard"
        )
...
def scatter_storage(
    ...
    Raises:
        ValueError: If dim or mesh_dim is out of range, if mesh_dim doesn't
            exist, or if the DTensor is not replicated on the target mesh dim.
    ...
    Example:
        ...
        >>> sharded = scatter_storage(param, dim=0, mesh_dim="dp")
        ...
        ... )
        >>> sharded = scatter_storage(param, dim=[0, 1], mesh_dim=["dp", "tp"])
        >>> sharded.shape  # (2, 2)
    ...
    for tensor_dim, mesh_dim_idx in zip(shard_dims, mesh_dim_indices_tuple):
        world_size = device_mesh.size(mesh_dim_idx)
    ...
    # For scatter_storage, the global shape is the full tensor shape (no TP involved)
...
def scatter_tensor_group(
    ...
    Example:
        ...
        >>> # Element mode (FSDP v1 style)
        >>> sharded = scatter_tensor_group(tensors, mesh, "dp", mode="element")

        >>> # Tensor mode (each tensor whole on one rank)
        >>> sharded = scatter_tensor_group(tensors, mesh, "dp", mode="tensor")
    """
