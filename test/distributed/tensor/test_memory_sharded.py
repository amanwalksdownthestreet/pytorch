...
class TestMemoryShardedDTensor(DTensorTestBase):
    ...
    def test_tensor_ops_not_supported(self):
        ...
        msdt = scatter_storage(dtensor, dim=0, mesh_dim=0)
...
class TestScatterStorage(DTensorTestBase):
    """Distributed tests for scatter_storage() factory function."""
    ...
    @with_comms
    def test_scatter_storage_basic(self):
        """Test basic scatter_storage creates MemoryShardedDTensor."""
        device_mesh = init_device_mesh(self.device_type, (self.world_size,))
    ...
    @with_comms
    def test_scatter_storage_dim0(self):
    ...
    @with_comms
    def test_scatter_storage_dim1(self):
        ...
        msdt = scatter_storage(dtensor, dim=1, mesh_dim=0)
    ...
    @with_comms
    def test_scatter_storage_2d_mesh(self):
        """Test scatter_storage with 2D mesh using mesh_dim name."""
        # Create 2D mesh: 2x2
        ...
        msdt = scatter_storage(dtensor, dim=0, mesh_dim="dp")
    ...
    @with_comms
    def test_scatter_storage_validation_dim(self):
        ...
        with self.assertRaises(ValueError):
            scatter_storage(dtensor, dim=5, mesh_dim=0)  # Out of range

    @with_comms
    def test_scatter_storage_validation_mesh_dim(self):
        ...
        with self.assertRaises(ValueError):
            scatter_storage(dtensor, dim=0, mesh_dim="invalid")  # Invalid name

    @with_comms
    def test_scatter_storage_negative_dim(self):
    ...
    @with_comms
    def test_scatter_storage_requires_replicated(self):
        """Test that scatter_storage raises error when placement is Shard."""
        device_mesh = init_device_mesh(self.device_type, (self.world_size,))
        ...
        with self.assertRaises(ValueError) as ctx:
            scatter_storage(sharded_dtensor, dim=0, mesh_dim=0)

        self.assertIn("Shard", str(ctx.exception))

    @with_comms
    def test_scatter_storage_rejects_partial(self):
        """Test that scatter_storage raises error when placement is Partial."""
        device_mesh = init_device_mesh(self.device_type, (self.world_size,))
        ...
        with self.assertRaises(ValueError) as ctx:
            scatter_storage(partial_dtensor, dim=0, mesh_dim=0)
...
class TestEdgeCases(DTensorTestBase):
    ...
    def test_1d_tensor(self):
        """Test scatter_storage works with 1D tensors."""
        device_mesh = init_device_mesh(self.device_type, (self.world_size,))
...
class TestIntegration(DTensorTestBase):
    ...
    def test_roundtrip_dim0(self):
        """Test scatter_storage -> unshard roundtrip on dim 0."""
        device_mesh = init_device_mesh(self.device_type, (self.world_size,))
    ...
    def test_roundtrip_dim1(self):
        """Test scatter_storage -> unshard roundtrip on dim 1."""
        device_mesh = init_device_mesh(self.device_type, (self.world_size,))
    ...
    def test_different_mesh_dim_names(self):
        ...
        msdt = scatter_storage(dtensor, dim=0, mesh_dim="data_parallel")
...
class TestTensorGroupStorage(DTensorTestBase):
    ...
    def test_scatter_tensor_group_convenience(self):
        ...
        # Tensor mode
        sharded = scatter_tensor_group(tensors, mesh, 0, mode="tensor")
        self.assertEqual(len(sharded), 4)
        for s in sharded:
            self.assertTrue(s.is_tensor_group_mode())

        # Element mode (default)
        sharded_elem = scatter_tensor_group(tensors, mesh, 0)  # default mode="element"
...
class TestDTensorInput(DTensorTestBase):
    ...
    def test_mixed_tensor_and_dtensor_input(self):
        ...
        # Shard using element mode
        sharded = scatter_tensor_group([plain_tensor, dtensor], mesh, 0, mode="element")
    ...
    def test_2d_mesh_dtensor_input(self):
        ...
        sharded = scatter_tensor_group([dtensor], mesh, "dp", mode="element")
